...
Well, as I am sure you know, SSH is all about two files, a private one you own and protect and a public one which you can give out willy-nilly. <span class="underline">Usually</span> those are `.ssh/id_rsa` and `.ssh/id_rsa.pub` but they can actually come from anywhere.
...
## Generate the keys and secure them {#generate-the-keys-and-secure-them}
...
## Copy the `.pub` to the client machine {#copy-the-dot-pub-to-the-client-machine}
...
## SSH using that key {#ssh-using-that-key}
...
Great, but what has this got to do with `rsync`? Well, `rsync` and <span class="underline">many</span> other things work over `ssh`. All we need to do is tell `rsync` which key to use:
