+++
title = "Blogging with cryogen (OLD)"
date = 2016-09-25
draft = false
+++
...
So, er yeah. I haven't really blogged for a <span class="underline">long</span> time - this time it will be different. I promise.
...
`jekyll` itself had moved to using `bundler` which I don't object to at all. The thing I <span class="underline">do</span> object to though is a gazillion =gem install=s, failing on the somewhat infamous [Nokogiri](<http://www.nokogiri.org/tutorials/installing_nokogiri.html>).
...
<span class="underline">make sure to create a new key pair for your remote server, `ssh-copy-id` to your remote server and then add the private key to your `circleci` project</span>
